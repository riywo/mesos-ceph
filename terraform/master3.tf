resource "aws_instance" "master3" {
	...
	private_ip        = "${var.master3_ip}"

	connection {
		user        = "ubuntu"
		key_file    = "${var.key_path}"
		host        = "${aws_instance.admin.public_ip}"
		script_path = "/tmp/${aws_instance.master3.id}.sh"
	}

	provisioner "file" {
		source      = "${path.module}/scripts/header.sh"
		destination = "/tmp/${aws_instance.master3.id}-00header.sh"
	}

	provisioner "file" {
		source      = "${path.module}/scripts/init_master.sh"
		destination = "/tmp/${aws_instance.master3.id}-01init_master.sh"
	}

	provisioner "remote-exec" {
		inline = [
			"echo main ${var.master3_ip} ${aws_instance.master3.public_dns} | cat /tmp/${aws_instance.master3.id}-*.sh - | bash"
		]
	}
}
